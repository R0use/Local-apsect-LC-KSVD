...
%       H_train         -label matrix for training feature 标签矩阵
...
numClass = size(H_train,1); % number of objects 38类
numPerClass = round(dictsize/numClass); % initial points from each classes 每类的初始分数 round函数返回一个四舍五入的值
Dinit = []; % for LC-Ksvd1 and LC-Ksvd2
dictLabel = [];
for classid=1:numClass
    col_ids = find(H_train(classid,:)==1);
    data_ids = find(colnorms_squared_new(training_feats(:,col_ids)) > 1e-6);   % ensure no zero data elements are chosen 向量求和
    %    perm = randperm(length(data_ids));
    perm = [1:length(data_ids)]; 
    %%%  Initilization for LC-KSVD (perform KSVD in each class)
    Dpart = training_feats(:,col_ids(data_ids(perm(1:numPerClass))));%Dpart是抽取每类前numPerClass个
